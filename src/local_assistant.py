...
SYSTEM_PROMPT_TEMPLATE = (
    ...
    "- If you suspect a medical emergency, you MUST advise the user to call local "
    "emergency services (e.g., 911, 999, 112) or go to the nearest ER immediately.\n"
    "- If you suspect signs of depression, self-harm, or suicidal ideation, you MUST "
    "maintain a compassionate tone and MANDATORILY provide these resources:\n"
    "  * US/Canada: Call or text 988 (Suicide & Crisis Lifeline).\n"
    "  * International: https://findahelpline.com or https://www.befrienders.org.\n"
    "- PROFESSIONAL ADVICE: You are not equipped to provide medical, legal, or other "
    "professional advice. For serious and consequential matters, you MUST recommend "
...
class LocalChatAssistant:
    ...
    def __init__(self):
        ...
        self.stop_signal: Optional[Callable[[], bool]] = None
    ...
    def stop_requested(self):
        """Checks if an interruption has been requested."""
    ...
    def _is_fact_known(self, entity, fact, all_facts):
        """Checks if a fact is already known either in context or database."""
        ...
        if not already_known:
            if self.memory.has_similar_fact(entity, fact):
                debug_print(f"[*] Memory: Blocking duplicate fact for '{entity}': {fact}")
                return True
        return already_known

    def _process_single_memory_op(self, op, all_facts):
        """Processes a single memory operation."""
        action, entity = op.get('op'), op.get('entity')
        fact = re.sub(r'\s*\[ID:\s*\d+\]$', '', op.get('fact', '')).strip()
        ...
        try:
            f_id = int(op.get('id')) if action != 'add' and op.get('id') is not None else None
        ...
        if action == 'add' and fact:
            if not self._is_fact_known(entity, fact, all_facts):
    ...
    def _apply_memory_operations(self, ops, all_facts):
        ...
        for op in [o for o in ops if isinstance(o, dict)]:
            if self._process_single_memory_op(op, all_facts):
                updated = True
        return updated

    def _extract_ops_for_update(self, user_input, assistant_response):
        """Helper to extract memory operations from LLM."""
        ...
        return ops, all_facts

    def _perform_memory_update_logic(self, user_input, assistant_response, start_count):
        """Internal logic for memory update."""
        if not self._should_process_memory(user_input):
            return False

        ops, all_facts = self._extract_ops_for_update(
            user_input, assistant_response
        )

        if self.memory.clear_count != start_count:
            debug_print("[*] Memory: Aborting update - memory was cleared.")
            return False

        updated = self._apply_memory_operations(ops, all_facts)

        if updated and self.memory.clear_count == start_count:
            self._cached_prompt = None
    ...
    def perform_memory_update(self, user_input, assistant_response, on_complete=None):
        ...
        self._run_memory_update_task(user_input, assistant_response, on_complete)

    def _run_memory_update_task(self, user_input, assistant_response, on_complete):
        """Internal task for memory update."""
        start_count = self.memory.clear_count
        updated = False
        try:
            updated = self._perform_memory_update_logic(
                user_input, assistant_response, start_count
            )
    ...
    def _get_decision_context(self, user_input, facts):
        """Helper to get context for search decision."""
        if facts is None:
    ...
    def _build_search_decision_msgs(self, user_input, facts):
        """Helper to build messages for search decision."""
        ctx = self._get_decision_context(user_input, facts)
        system_msg = (
            "You are a skeptical Search Coordinator. Determine if external data "
            "is REQUIRED to answer the user's current query.\n\n"
            "STRICT PROTOCOLS:\n"
            "1. REAL-TIME DATA: For live news, current weather, stock prices, and today's events,\n"
            "a search is ALWAYS REQUIRED to ensure absolute accuracy. Memory is not enough.\n"
            "2. PERSONAL/STATIC FACTS: If the query is about user preferences, identity,\n"
            "or static historical/scientific facts, and the answer is in 'MEMORY FACTS',\n"
            "you MUST return 'done'. Do not search for what you already know.\n\n"
            "SEARCH REQUIRED IF:\n"
            "1. Real-time data is needed (news, weather, finance, today's events).\n"
            "2. USER EXPLICITLY ASKS to 'search', 'look up', 'find' or 'check' on the web.\n"
            "3. You lack specific facts that cannot be derived from context or MEMORY FACTS.\n"
            "4. Query involves specific data from after your training cutoff (late 2020).\n\n"
            "DO NOT SEARCH IF:\n"
            "1. THE ANSWER IS IN MEMORY FACTS and is NOT a real-time factual state.\n"
            "2. ROLEPLAY/CREATIVE: Use internal knowledge for fictional contexts.\n"
            "3. CHAT/GREETINGS: Small talk, philosophy, or meta-conversation.\n"
            "4. GENERAL KNOWLEDGE: Static facts already in your training data.\n"
            "5. CRISIS: Medical or mental health emergencies (use Crisis Protocol instead).\n\n"
            "TYPES: 'web', 'news', 'weather', 'finance', 'roleplay', 'crisis', 'none'.\n\n"
            "Use 'web' for general information, facts, and website lookups.\n"
            "Use 'news' ONLY for current events, breaking news, or headlines.\n\n"
            "FORMAT: JSON only. {\"action\": \"search\" or \"done\", \"query\": \"str\", "
            "\"symbol\": \"ticker|null\", \"type\": \"one of the TYPES above\"}\n"
            "CRITICAL: The 'action' field MUST be either 'search' or 'done'. "
            "NEVER put the search type (like 'news') into the 'action' field."
        )
        user_msg = (
            f"MEMORY FACTS:\n{ctx['fact_text']}\n\n"
            f"Current Date: {datetime.now().strftime('%Y-%m-%d')}\n"
            f"RECENT SEARCHES: {ctx['rec_queries']}\n"
            f"SEARCH HISTORY: {ctx['recent_ctx']}\n"
            f"User Input: {user_input}"
        )
        return [{"role": "system", "content": system_msg},
                {"role": "user", "content": user_msg}]

    def _parse_search_decision(self, response_text):
        """Helper to parse and sanitize search decision."""
        data = json.loads(response_text.strip())
        valid_types = {'web', 'news', 'weather', 'finance', 'roleplay', 'crisis'}
        action = str(data.get("action", "")).lower()
        orig_type = str(data.get("type", "")).lower()

        if action in valid_types:
            data["type"], data["action"] = action, "search"
        elif action == "search" and (not orig_type or orig_type == "none"):
            data["type"] = "web"
        elif action not in ("search", "done"):
            data["action"] = "done"

        if data.get("action") == "search":
            msg = (f"[*] Search Decision: {data.get('action')} "
                   f"({data.get('type')}) - Query: {data.get('query')}")
            debug_print(msg)
        else:
            debug_print(f"[*] Search Decision: {data.get('action')}")
        return data

    def _get_search_decision(self, user_input, facts=None):
        """Asks the model if a web search is needed."""
        msgs = self._build_search_decision_msgs(user_input, facts)
        try:
            ...
            if hasattr(res, '__iter__') and not isinstance(res, (dict, str)):
                for chunk in res:
                    if self.stop_requested:
                        debug_print("[*] Search Decision Interrupted")
                        return {"action": "done", "type": "none"}
                    resp_text += chunk.get('message', {}).get('content', '')
            else:
                resp_text = (res.get('message', {}).get('content', '')
                             if isinstance(res, dict) else str(res))

            return self._parse_search_decision(resp_text)
    ...
    def _auto_scrape(self, user_input, results, is_news):
        ...
        for url in re.findall(r'Source: (https?://\S+)', results):
            ...
            if url not in seen_urls:
                ...
                raw_text = self._run_interruptible(SearchEngine.scrape_url, url)
                if self.stop_requested or raw_text is None:
                    break
    ...
    def _handle_scraping(self, user_input, results, recent_ctx, **flags):
        ...
        result_info = ""
        try:
            if flags.get('is_news') or flags.get('is_finance'):
                result_info = self._auto_scrape(user_input, results, flags.get('is_news'))
            else:
                resp_text = self._get_scrape_decision(user_input, results, recent_ctx)
                data = json.loads(resp_text.strip())
                if data.get("action") == "scrape":
                    url = data.get("url")
                    if url and url.startswith("http") and not self.stop_requested:
                        raw_text = self._run_interruptible(SearchEngine.scrape_url, url)
                        if not self.stop_requested and raw_text is not None:
                            result_info = self._distill_information(user_input, url, raw_text)
        except (ollama.ResponseError, json.JSONDecodeError, AttributeError) as exc:
            debug_print(f"[*] Scraping Decision Failed: {exc}")
        finally:
            Profiler().stop("Search Scraping")
        return result_info

    def _get_scrape_decision(self, user_input, results, recent_ctx):
        """Helper to get scrape decision from LLM."""
        system_scrape = (
        ...
        if hasattr(res, '__iter__') and not isinstance(res, (dict, str)):
            for chunk in res:
                if self.stop_requested:
                    return ""
                resp += chunk.get('message', {}).get('content', '')
        elif isinstance(res, dict):
            resp = res.get('message', {}).get('content', '')
        else:
            resp = str(res)
        return resp
    ...
    def _distill_information(self, user_input, url, raw_text):
        ...
        try:
            ...
            if hasattr(res, '__iter__') and not isinstance(res, (dict, str)):
                for chunk in res:
                    if self.stop_requested:
                        debug_print("[*] Distillation Interrupted")
                        break
                    info += chunk.get('response', '')
            elif isinstance(res, dict):
            ...
            debug_print(f"[*] Distillation Failed: {exc}")
            return f"\n\n--- RELEVANT DATA FROM {url} ---\n[Distillation failed]"

    def _run_interruptible(self, func, *args, **kwargs):
        """Runs a blocking function in a background thread with interrupt polling."""
        executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
        try:
            future = executor.submit(func, *args, **kwargs)
            while not future.done():
                if self.stop_requested:
                    # Shutdown without waiting for the blocked thread
                    executor.shutdown(wait=False, cancel_futures=True)
                    return None
                time.sleep(0.05)
            return future.result()
        except (RuntimeError, ValueError, TypeError, AttributeError) as exc:
            debug_print(f"[*] Interruptible call failed: {exc}")
            return None
        finally:
            executor.shutdown(wait=False)

    def prepare_turn(self, user_input, skip_search=False):
        """Runs memory retrieval and search gatekeeper."""
        Profiler().reset()
        search_context = None
        decision_data = None

        # Step 1: Memory Lookup (Mandatory first step)
        Profiler().start("Memory Lookup")
        facts = self.memory.get_relevant_facts(user_input)
        Profiler().stop("Memory Lookup")

        if self.stop_requested:
            return {"facts": facts, "search_context": None, "search_type": "none"}

        # Step 2: Search Decision (Uses facts from Step 1)
        filler = {"hi", "hello", "hey", "thanks", "ok", "yes", "no"}
        clean_in = re.sub(r'[^a-z\s]', '', user_input.lower()).strip()

        # Heuristic skip for obvious small talk
        if not (skip_search and clean_in in filler and len(user_input) < 10):
            Profiler().start("Search Decision")
            decision_data = self._run_interruptible(
                self._get_search_decision, user_input, facts=facts
            )
            Profiler().stop("Search Decision")

            if self.stop_requested or decision_data is None:
                return {"facts": facts, "search_context": None, "search_type": "none"}
            ...
            if decision_data and decision_data.get("action") == "search":
                ...
                try:
                    # Note: decide_and_search no longer calls on_start/on_end
    ...
    def _perform_search(self, user_input, data):
        ...
        if not is_spec_no_wiki:
            wiki_res = self._run_interruptible(SearchEngine.wikipedia_search, base_query)

        if wiki_res or self.stop_requested:
            if wiki_res:
    ...
    def _try_specialized_search(self, query, data):
        ...
        if search_type == "finance" and symbol and symbol.lower() != "null":
            Profiler().start("Search")
            res = self._run_interruptible(SpecializedSearch.get_ticker_data, symbol)
            Profiler().stop("Search")
        elif search_type == "weather":
            Profiler().start("Search")
            res = self._run_interruptible(SpecializedSearch.get_weather, query)
            Profiler().stop("Search")
    ...
    def _handle_news_search(self, query):
        ...
        Profiler().start("Search")
        res = self._run_interruptible(SpecializedSearch.get_news, news_query)
        Profiler().stop("Search")
        return res
    ...
    def _perform_web_search(self, user_input, base_query, data):
        ...
        results = self._run_interruptible(SearchEngine.web_search, query)
        Profiler().stop("Search")

        if self.stop_requested or results is None:
            return None
